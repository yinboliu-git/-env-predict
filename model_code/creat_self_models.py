...
class MaxEnt:
    def __init__(self, epsilon=1e-10, maxstep=100):
        self.epsilon = epsilon
        self.maxstep = maxstep
        self.w = None  # 特征函数的权重
        self.labels = None  # 标签
        self.fea_list = []  # 特征函数
        self.px = defaultdict(int)  # 经验边缘分布概率
        self.pxy = defaultdict(int)  # 经验联合分布概率,由于特征函数为取值为0，1的二值函数，所以等同于特征的经验期望值
        self.exp_fea = defaultdict(int)  # 每个特征在数据集上的期望
        self.data_list = []  # 样本集，元素为tuple((X),y)
        self.N = None  # 样本总量
        self.M = None  # 某个训练样本包含特征的总数，这里假设每个样本的M值相同，即M为常数。其倒数类似于学习率
        self.n_fea = None  # 特征函数的总数
        self.nn = np.power(10,2.5)
    ...
    def fit(self, X_data, y_data):
        ...
        self.x_min = X_data.min(axis=0)
        self.x_max = X_data.max(axis=0)
        X_data = (X_data-self.x_min)/(self.x_max-self.x_min)
        X_data = X_data*self.nn
